...
class LocatorPlus {
	...
	renderResultsList() {
		...
		if (this.searchLocation) {
			console.log(locations[0].travelDistanceText)
			this.sectionNameEl.textContent =
				'Nearest locations (' + this.locations.length + ')'
			locations.forEach((location) => {
				location.distance = this.getLocationDistance(location)
			})
			locations = locations.filter((location) => {
				// if (location.travelDistanceText != null) {
				// 	console.log(parseFloat(location.travelDistanceText))
				// 	return location
				// }
				// return location.travelDistanceText.replace(/ mi$/, '')
				// }
				// return location
				return (
					location.travelDistanceText !== null &&
					parseFloat(
						location.travelDistanceText.replace(/ mi$/, '')
					) <= 5
				)
			})
			console.log(locations, 'below 5?')
			locations.sort((a, b) => {
