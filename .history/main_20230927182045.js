		...
		const callback = (response, status) => {
			if (status === 'OK') {
				...
				for (let i = 0; i < distances.length; i++) {
					...
					console.log(location.travelDistanceValue)
				}

				// Filter locations based on MAX_DISTANCE_METERS
				this.locations = this.locations.filter((location) => {
					console.log('hi')
					return location.travelDistanceValue <= MAX_DISTANCE_METERS
				})

				// Now you can re-render the results list, with the ordering preserved.
				this.renderResultsList()
			}
		}
