...
class LocatorPlus {
	...
	static MAX_LOCATIONS_TO_SHOW = 50
	...
	updateTravelTimes() {
		...
		let processedChunks = 0 // Track the number of chunks processed
				...
				(response, status) => {
					if (status === 'OK') {
						...
						for (let i = 0; i < distances.length; i++) {
							...
							const location =
								this.locations[i + chunkIndex * 100]
						...
						if (processedChunks === destinationChunks.length) {
							// Filter and render logic after all chunks have been processed
